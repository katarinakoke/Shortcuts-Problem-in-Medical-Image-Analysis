...
def main():
	...
	for filename in os.listdir('.'):
		...
		if filename.endswith('.csv'):
			...
			y_score = file['y_score_Problem_in']
			y_true = file['Pneumothorax'].map({1.: 1 , 0.: 0, -1.: 0}).fillna(0)
			print(y_true)
			sens_features = file['Sex']

			sens_y_pred = file['sensitive_y_pred_Problem_in']
			sens_y_score = file['sensitive_y_score_Problem_in']
			sens_y_true = file['Sex'].map({'Female': 1 , 'Male': 0, 'Unknown': 0}).fillna(0)
			...
			auc = area_under_the_curve(y_true, y_score)
			sens_auc = area_under_the_curve(sens_y_true, sens_y_score)
	...
	# Datasets
	results_df['TrainDataset'] = results_df['Model Name'].str.split('_').str[2]
	results_df['TestDataset'] = results_df['Model Name'].str.split('_').str[1]
