...
def get_embedding(cfg, args, model, dataloader):
    ...
    for step in range(steps):
        ...
        logits, logit_map, sensitive_target, sensitive_feat = model(image)
        embeddings.append(sensitive_feat.cpu().detach().numpy())
    return embeddings
...
def run(args):
    ...
    # save embedding
    np.save('sensitive_embedding_chexpert.npy', new_embs)
