...
def get_loss(output, target, sensitive_logits, sensitive_target, index, device, cfg):
    ...
    if cfg.sensitive_criterion == 'BCE':
        for num_class in cfg.num_classes:
            assert num_class == 1
        sensitive_target = sensitive_target.view(-1)
        sensitive_logits = sensitive_logits.view(-1)
        
        sensitive_pos_weight = torch.from_numpy(
            np.array(cfg.sensitive_pos_weight,
                     dtype=np.float32)).to(device).type_as(target)
        if cfg.batch_weight:
            if sensitive_target.sum() == 0:
                sensitive_loss = torch.tensor(0., requires_grad=True).to(device)
            else:
                weight_sensitive = (sensitive_target.size()[0] - sensitive_target.sum()) / sensitive_target.sum()
                sensitive_loss = F.binary_cross_entropy_with_logits(sensitive_logits, sensitive_target, 
                pos_weight=weight_sensitive)
        else:
            sensitive_loss = F.binary_cross_entropy_with_logits(sensitive_logits, sensitive_target, 
                pos_weight=weight_sensitive)

        sensitive_label = torch.sigmoid(sensitive_logits[index].view(-1)).ge(0.5).float()
...
def train_epoch(summary, summary_dev, cfg, args, model, dataloader,
    ...
    sensitive_acc_sum = np.zeros(num_tasks)
    ...
    for step in range(steps):
        ...
        for t in range(num_tasks):
            ...
            sensitive_acc_sum[t] += sensitive_acc.item()
        ...
        if summary['step'] % cfg.log_every == 0:
            ...
            sensitive_acc_str = ' '.join(map(lambda x: '{:.3f}'.format(x), sensitive_acc_sum))

            logging.info(
                '{}, Train, Epoch : {}, Step : {}, Loss : {}, '
                'Acc : {}, Sensitive_Loss : {}, Sensitive_Acc : {}, Run Time : {:.2f} sec'
                .format(time.strftime("%Y-%m-%d %H:%M:%S"),
                        summary['epoch'] + 1, summary['step'], loss_str,
                        acc_str, sensitive_loss_str, sensitive_acc_str, time_spent))
            ...
            for t in range(num_tasks):
                ...
                summary_writer.add_scalar(
                    ...
                    summary['step'])
                summary_writer.add_scalar(
                    'train/sensitive_acc_{}'.format(label_header[t]), sensitive_acc_sum[t],
                    summary['step'])
        ...
        if summary['step'] % cfg.test_every == 0:
            ...
            if save_best:
                torch.save(
                    ...
                    os.path.join(args.save_path, 'Best_Balanced_Sex_2_neg005{}.ckpt'.format(
                        best_dict['best_idx']))
                )
...
def test_epoch(summary, cfg, args, model, dataloader):
    ...
    sensitive_predlist = list(x for x in range(len(cfg.num_classes)))
    sensitive_true_list = list(x for x in range(len(cfg.num_classes)))
    ...
    summary['sensitive_acc'] = sensitive_acc_sum / steps
...
def run(args):
    ...
    summary_dev = {'loss': float('inf'), 'acc': 0.0, 'sensitive_loss': float('inf'), 'sensitive_acc': 0.0}
        ...
        if save_best:
            torch.save(
                ...
                os.path.join(args.save_path,
                             'Best_Balanced_Sex_2_neg005{}.ckpt'.format(best_dict['best_idx']))
            )
