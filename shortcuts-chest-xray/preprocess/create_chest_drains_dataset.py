...
df = pd.merge(ChestX_ray14, drain_annotations, on='Image Index', how='outer')

# Adding a new column for Pneumothorax labels only
df['Pneumothorax'] = df['Finding Labels'].str.contains('Pneumothorax').astype(int)
...
df.to_csv('../chest_drains_df.csv', index=False)
