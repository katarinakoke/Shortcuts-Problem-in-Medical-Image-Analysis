...
df_no_pneumothorax = pd.read_csv('datasets/chest_drains_annotations_without_pneumothorax.csv')
df_pneumothorax = pd.read_csv('datasets/NIH-CX14_TubeAnnotations_NonExperts_aggregated.csv', index_col=0)
...
df_no_pneumothorax['Folder'] = df_no_pneumothorax['image'].apply(extract_folder)

df  = df_original.merge(df_no_pneumothorax,on='Image Index', how='outer')
df  = df.merge(df_pneumothorax,on='Image Index', how='outer')

df.to_csv('../chest_drains_annotations_duplicates.csv', index=False)
...
# Condition to drop rows where 'folder' is 0 and 'pneumothorax' is 1
condition = ~((df['Folder'] == 0)) #& (df['Pneumothorax'] == 1))

# Apply the condition to filter the DataFrame
df = df[condition]

df = df.drop(['Drain_x','Drain_y','OriginalImage[Width', 'Height]', 'OriginalImagePixelSpacing[x', 'y]', 'Unnamed: 11', 'Drain_a1', 'Drain_Location_a1', 'Drain_a2', 'Drain_Location_a2', 'annotation_id','annotator','created_at','id','image','lead_time','updated_at', 'Folder'], axis=1)
